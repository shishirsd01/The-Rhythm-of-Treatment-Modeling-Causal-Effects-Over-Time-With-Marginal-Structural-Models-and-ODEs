...
# Plotting Bar graph - compare ODE+MSM vs Only ODE
...
def prep(df, dt=DT_MIN):
    ...
    for _, g in df.sort_values("patient_id").groupby("patient_id", sort=False):
        ...
        # smooth / interpolate key signals
...
def gps_weights(df,
    ...
    # mean functions for each treatment dimension
    mu = np.column_stack([
...
def residualize(df_w):
    X = sm.add_constant(
        df_w[["basal_rate","bolus_volume_delivered","carb_input",
...
def evaluate(df, direct, msm,
    ...
    per = {
        h: {k: [] for k in
            ["rmse_direct","rmse_msm","mae_direct","mae_msm",
    ...
    for _, g in df.groupby("patient_id", sort=False):
        ...
        c     = g["carb_input"].to_numpy()
        ba    = g["basal_rate"].to_numpy()
        bo    = g["bolus_volume_delivered"].to_numpy()

        starts = np.arange(0, len(g) - max(horizons), stride)

        for h in horizons:
            d_obs, d_dir, d_msm = [], [], []
            meal_o, meal_p, bol_o, bol_p = [], [], [], []
            ...
            for t0 in starts:
                ...
                if c[t0] >= meal_thr:
                    meal_o.append(d_obs[-1])
                    meal_p.append(d_msm[-1])
                if bo[t0] >= bolus_thr:
                    bol_o.append(d_obs[-1])
                    bol_p.append(d_msm[-1])

            if not d_obs:
                continue

            d_obs = np.array(d_obs)
            d_dir = np.array(d_dir)
            d_msm = np.array(d_msm)
            ...
            if meal_o:
                per[h]["sign_meal_msm"].append(sacc(meal_p, meal_o))
            if bol_o:
                per[h]["sign_bolus_msm"].append(sacc(bol_p, bol_o))
...
def plot_rmse(summary, savepath=RMSE_FIG_PATH):
    """
    Plot RMSE panels with:
      - a common y-axis across all horizons (heights aligned)
      - y-limits high enough so 30 and 60 min Direct bars + CIs are not truncated
    """
    hs = sorted(summary.keys())   # e.g. [3, 6, 12]

    # Extract means and 95% CI half-widths
    ...
    # If nothing to plot, just return
    if not hs:
        print("No RMSE values to plot.")
        return

    # ---- Compute a common y-limit for all panels ----
    # Max over (mean + CI) across both methods and all horizons
    all_tops = []
    ...
    # Add 15% headroom so nothing touches the top
    ymax = max(all_tops) * 1.15
    ymin = 0.0

    # ---- Create panels with a shared y-axis ----
    fig, axes = plt.subplots(1, len(hs), figsize=(11, 3.4), sharey=True)

    # If only one horizon, axes is a single Axes object, not an array
    if len(hs) == 1:
        axes = [axes]
    ...
    for i, (ax, h, dv, mv, civ, cim) in enumerate(
        ...
        methods = ["Direct", "MSM+ODE"]
        means   = [dv, mv]
        cis     = [civ, cim]

        ax.bar(methods, means, yerr=cis, capsize=4)

        # Set common y-limits (prevents truncation and misalignment)
...
if __name__ == "__main__":
    ...
    print("Estimating GPS weights and residualizing glucose...")
    df_w = gps_weights(df)
